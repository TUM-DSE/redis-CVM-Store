...
/* ---------------------------------------------------------------------------
 * AccelStore RDB snapshots: each generation is a store LOG "<rdb_filename>.<gen>".
 * The commit point is the pointer file "<rdb_filename>.accel-current" holding
 * the generation number (0 or absent = no snapshot). Children serialize into
 * a pipe drained by a parent thread.
 * ------------------------------------------------------------------------ */
...
/* Commit point replacing rename(): repoint the pointer file at 'gen', then drop
 * the old log. Once the rename succeeded the commit is taken even if C_ERR is returned. */
...
/* Serialize the dataset into the drainer pipe; durability is the final accelFsync(). */
...
/* Clean up a background save whose done handler never ran (shutdown killed the child). */
...
int rdbSave(int req, char *filename, rdbSaveInfo *rsi, int rdbflags) {
    ...
    if (accelEnabled()) {
        ...
        /* A stale partial log means a previous attempt crashed before committing. */
...
int rdbSaveBackground(int req, char *filename, rdbSaveInfo *rsi, int rdbflags) {
    ...
    if (accelEnabled()) {
        /* The store side is prepared in the parent; the child only sees the pipe. */
        accelRdbReapStaleBackgroundSave();
    ...
    if ((childpid = redisFork(CHILD_TYPE_RDB)) == 0) {
        ...
        if (accelEnabled()) {
            ...
            if (accelStartDrainer(accel_pipe[0],accel_asfd) == 0) {
                ...
                /* No reader: the child's writes fail with EPIPE and the done handler fails the run. */
                serverLog(LL_WARNING,
...
void rdbRemoveTempFile(pid_t childpid, int from_signal) {
    ...
    /* No temp files: the snapshot streams straight into the store. */
...
static void backgroundSaveDoneHandlerDisk(int exitcode, int bysignal, time_t save_end) {
    if (accelEnabled() && accel_rdb_child_name != NULL) {
        ...
        /* The child exited, so the drainer is at EOF and the join cannot block. */
