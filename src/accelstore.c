...
int accelInit(void) {
    ...
    /* Persist the trusted counter across restarts, beside the data files in
     * the working directory (Redis chdir'd to server.dir in initServer). A
     * reopened store is checked against this value - restarting without it
     * is a freshness violation and the store refuses to load. */
    opts.tc_persist_path = "accelstore.tc";
    opts.sync_opts = sync_opts;
    opts.sync_opts_count = sizeof(sync_opts) / sizeof(sync_opts[0]);
    /* Big enough for one streamed segment plus headroom: appends larger than
     * ACCEL_SEGMENT_BYTES are split by accelLogAppend(). */
...
int accelLogExists(const char *name) {
    /* Cheapest existence probe the ABI offers: a lazy open builds no cache. */
...
ssize_t accelLogAppend(int asfd, const void *buf, size_t len) {
    /* One LOG entry per aof_buf flush / streamed segment; a flush larger than
     * a segment becomes several entries. The concatenated payload stream is
     * what the on-disk file would have held, so the read side is unchanged. */
    size_t done = 0;
    while (done < len) {
        size_t n = len - done;
        ...
        if (rc != 0) {
            ...
            errno = -rc > 0 ? -rc : EIO;
            /* All-or-nothing per entry: report what landed, like write(2). */
...
static int accelStreamSeek(void *cookie, off64_t *offset, int whence) {
    ...
    if (whence == SEEK_SET && *offset == 0) {
        /* The AOF loader's rewind after the 5-byte magic sniff. */
        s->next = 0;
        s->len = s->off = 0;
        s->logical = 0;
        return 0;
