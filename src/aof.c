...
/* In-flight AOFRW state under the AccelStore backend. The new BASE log is
 * created in the store under its final name before forking, the child streams
 * the serialization into a pipe and the parent's drainer thread appends it to
 * that log, so the child never touches the store. */
...
/* Forget the in-flight rewrite BASE log state. If 'delete_log' is set the
 * (partial) BASE log is also removed from the store. Safe to call when no
...
int aofDelHistoryFiles(void) {
    ...
    while ((ln = listNext(&li)) != NULL) {
        ...
        if (accelEnabled()) {
            /* Deleting a store log is pure metadata, do it inline. */
...
void aofOpenIfNeededOnServerStart(void) {
    ...
    if (accelEnabled()) {
        /* The store LOG is append-only by construction; create it when the
         * manifest references an INCR that does not exist yet (O_CREAT). */
...
int openNewIncrAofForAppend(void) {
    ...
    if (accelEnabled()) {
        /* O_TRUNC semantics: a log with this name can be left over from a
         * crashed run whose manifest update never landed. */
...
int startAppendOnly(void) {
    ...
    if (accelEnabled()) {
        /* The AOF_WAIT_REWRITE temp-INCR rename dance has no store
         * equivalent. Boot-time "appendonly yes" does not come through here
         * (see aofOpenIfNeededOnServerStart). */
...
ssize_t aofWrite(int fd, const char *buf, size_t len) {
    ...
    /* accelLogAppend already loops over the store's entry-size limit and
     * mirrors write(2)'s short-write contract: on failure it returns the
     * bytes that landed (or -1 with errno when nothing did). */
...
void flushAppendOnlyFile(int force) {
    ...
    if (nwritten != (ssize_t)buflen) {
        ...
        if (nwritten == -1) {
            ...
            if (accelEnabled()) {
                /* Store entries are all-or-nothing: whatever accelLogAppend
                 * reported as written is a complete, durable-entry prefix, so
                 * there is nothing to truncate. Keep the prefix and retry the
                 * tail later, as with a failed ftruncate below. */
...
try_fsync:
    ...
    if (server.aof_fsync == AOF_FSYNC_ALWAYS) {
         ...
         * Under the accelstore backend the store's global durability barrier
         * plays the fsync role. */
        if ((accelEnabled() ? accelFsync() : redis_fsync(server.aof_fd)) == -1) {
...
uxeof: /* Unexpected AOF end of file. */
    if (accelEnabled()) {
        /* Store entries are atomic, so a torn tail is impossible: a short
         * read here means the log is really damaged, never truncate it. */
...
cleanup:
    ...
    if (accelEnabled()) {
        /* fileno() on the store's fopencookie stream is -1 and there is no
         * page cache to reclaim: just close the stream. */
        fclose(fp);
    } else {
...
/* Serialize the whole dataset into 'aof' in the format of a BASE file:
 * an RDB preamble when aof-use-rdb-preamble is enabled, a command stream
 * otherwise. This is the target-independent middle of an AOF rewrite: the
 * file path wraps it in a temp file plus rename, the accelstore path streams
 * it into a pipe drained into the store. */
...
/* accelstore variant of rewriteAppendOnlyFile(): serialize the whole dataset
 * into the store LOG named 'base_name'. The log is created directly under its
 * final name (no temp file and no rename), and the serialization streams
 * through a pipe drained by the store drainer thread, so this in-process path
 * writes to the store exactly the way the forked rewrite's parent does. */
static int rewriteAppendOnlyFileToAccelLog(const char *base_name) {
    rio aof;
    int pipefd[2];
    int ret = C_OK;
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if (accelEnabled()) {
        /* The child never touches the store: the new BASE log is created in
         * the parent, directly under the name the done handler will assign
         * (getNewBaseFileNameAndMarkPreAsHistory()'s formatting over the next
         * base file seq), and the child streams its serialization into a pipe
         * whose read end a drainer thread in the parent appends to that log. */
    ...
    if ((childpid = redisFork(CHILD_TYPE_AOF)) == 0) {
        ...
        if (accelEnabled()) {
            ...
            if (accelStartDrainer(accel_pipe[0], accel_rewrite_base_asfd) == -1) {
                serverLog(LL_WARNING, "Can't start the AOF store drainer thread");
                close(accel_pipe[0]);
                /* killAppendOnlyChild() reaps the child and, through
                 * aofRemoveTempFile(), reclaims the partial base log. */
                killAppendOnlyChild();
                server.aof_lastbgrewrite_status = C_ERR;
                return C_ERR;
            }
...
void aofRemoveTempFile(pid_t childpid) {
    ...
    if (accelEnabled()) {
        /* No temp files under the accelstore backend: reclaim the partial
         * BASE log of an aborted rewrite instead. The child has exited by
         * now, so a still-running drainer is at EOF and joinable. Safe to
         * call with no rewrite in progress. */
...
static void backupStartCommand(client *c) {
    if (accelEnabled()) {
        /* BACKUP pins BASE/INCR files with hard links, which have no store
         * equivalent. */
...
void backgroundRewriteDoneHandler(int exitcode, int bysignal) {
    ...
    /* The AOFRW child has exited, so its end of the pipe is closed and the
     * store drainer is at EOF: join it before judging the rewrite. */
    ...
    if (!bysignal && exitcode == 0) {
        ...
        if (accelEnabled()) {
            /* The BASE log was created in the store directly under its final
             * name before the fork, there is nothing to rename. */
        ...
        /* The manifest now names the new BASE log: the in-flight rewrite state
         * must be forgotten before aofRemoveTempFile() runs at cleanup, which
         * would otherwise delete the installed log. */
        if (accelEnabled()) {
            sdsfree(accel_rewrite_base_name);
            accel_rewrite_base_name = NULL;
        }
